...
public class UserInnerService {
    ...
    public int add(UserReqDto userReqDto) throws RuntimeException {
        System.out.println(FastjsonUtils.toJSONString(userReqDto));
...
//        return ret;
    ...
    private Set<UserRole> checkRole(List<String> roles, Integer userId) {
        if (CollectionUtils.isEmpty(roles)) {
            throw new MicroserviceException("角色信息不能为空");
        }
        Set<UserRole> userRoles = Sets.newHashSet();
        roles.stream().filter(roleId -> roleInnerService.selectById(Integer.parseInt(roleId)) != null)
                .forEach(roleId -> {
    ...
    private boolean checkUser(User user) {
        ...
        if (!StringUtils.isBlank(user.getEmail()) && !StringUtil.checkEmail(user.getEmail()))
            throw new MicroserviceException("邮箱格式无效。");
        return true;
    }
}
